...
class DatabaseWrapperTest {
    ...
    fun testAddTemporarySlot() {
        val slot = Slot(SlotCode("TEMP_SLOT"), Priority.NORMAL, Date(123), Duration.ofMillis(123), Date(123))
    ...
    fun testdeleteAllTemporarySlot() {
        ...
        val getTempSlotsQuery =
            connection.prepareStatement(
                "SELECT * FROM Slot WHERE is_temporary = ?"
            )
    ...
    fun testSaveSlots() {
        ...
        val slots = listOf<Slot>(Slot(SlotCode("TEST_TEST"), Priority.FIX_APPOINTMENT, Date(321), Duration.ofMillis(321), Date(321)))
        assertEquals(testObj.saveSlots(slots, QueueId(1)), true)
        val getTempSlotsQuery =
            connection.prepareStatement(
                "SELECT * FROM Slot WHERE code = ?"
            )
        ...
        //Weird behaviour
        /*
        assertEquals(Date(rs.getTimestamp("approx_time").time).time.milliseconds, Date(321).time.milliseconds)
    ...
    fun testChangeManagementPassword() {
        ...
        val getManagementPasswordQuery =
            connection.prepareStatement(
                "SELECT password FROM Management WHERE username = ?"
            )
        ...
        val resetManagementPasswordUpdate =
            connection.prepareStatement(
                "UPDATE Management SET password = ? WHERE username = ?"
            )
