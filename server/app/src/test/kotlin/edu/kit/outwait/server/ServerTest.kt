...
class SocketMock {
    ...
    init{
        mSocket.on(Socket.EVENT_CONNECT, Emitter.Listener {
            println("Callback EVENT_CONNECT")
        }
        )
        ...
        mSocket.on(Socket.EVENT_ERROR, Emitter.Listener {
            println("Callback EVENT_ERROR")
        }
        )
        mSocket.on(Socket.EVENT_CONNECT_TIMEOUT, Emitter.Listener {
            println("Callback EVENT_CONNECT_TIMEOUT")
        }
        )

        mSocket.on(Socket.EVENT_DISCONNECT, Emitter.Listener {
            println("Callback EVENT_DISCONNECT")
        }
        )

        for(e in Event.values()) {
            mSocket.on(e.getEventTag(), Emitter.Listener {
