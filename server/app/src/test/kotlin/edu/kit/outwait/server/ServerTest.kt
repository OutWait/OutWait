...
class SocketMock {
    ...
    private val mSocket = IO.socket(URI.create("http://127.0.0.1:8080/client"), options)
    private val LOG_ID = "SOCKET-MOCK"

    init {
        mSocket.on(
            Socket.EVENT_CONNECT,
            Emitter.Listener { Logger.debug(LOG_ID, "Callback EVENT_CONNECT") }
        )

        mSocket.on(
            Socket.EVENT_CONNECT_ERROR,
            Emitter.Listener {
                Logger.debug(LOG_ID, "Callback EVENT_CONNECT_ERROR")
                Logger.debug(LOG_ID, "Callback EVENT_CONNECT_ERROR")
            }
        )
        mSocket.on(
            Socket.EVENT_ERROR,
            Emitter.Listener { Logger.debug(LOG_ID, "Callback EVENT_ERROR") }
        )
        mSocket.on(
            Socket.EVENT_CONNECT_TIMEOUT,
            Emitter.Listener { Logger.debug(LOG_ID, "Callback EVENT_CONNECT_TIMEOUT") }
        )

        mSocket.on(
            Socket.EVENT_DISCONNECT,
            Emitter.Listener { Logger.debug(LOG_ID, "Callback EVENT_DISCONNECT") }
        )
        ...
        for (e in Event.values()) {
            mSocket.on(
                ...
                Emitter.Listener {
                    ...
                    for (d in it) Logger.debug(LOG_ID, d.toString())
                    // The mocking stuff
                    if (e == Event.READY_TO_SERVE) {
                        ...
                        Logger.debug(LOG_ID, "Send listen request")
                    }
                }
            )
        }
